	...
	for(;;) {
		...
		for (uint64_t i = 0; i < (uint64_t) NUM_BLOCKS*NUM_THREADS; ++i) {
			if (RESULT_THREAD_LOWEST_TEMP[5*i+1] < RESULT_LOWEST[1]) {
				...
				sprintf(buf, "Thread #%ld found the least value: %08x%08x%08x%08x%08x (nonce: %s)", i, RESULT_LOWEST[0], RESULT_LOWEST[1], RESULT_LOWEST[2], RESULT_LOWEST[3], RESULT_LOWEST[4], buf_nonce);
				log(buf);
		...
		if (1e9*10 < (double)elapsed_log.count()) {
			...
			sprintf(buf, "Current lowest: %08x%08x%08x%08x%08x (nonce: %s)", RESULT_LOWEST[0], RESULT_LOWEST[1], RESULT_LOWEST[2], RESULT_LOWEST[3], RESULT_LOWEST[4], buf_nonce);
			log(buf);
		}
	}
}