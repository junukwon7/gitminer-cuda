...
#define R0(l, m, n, o, p, q)                                     \
	p += ((m & (n ^ o)) ^ o) + blk0(q) + 0x5a827999 + rol(l, 5); \
	m = rol(m, 30);
#define R1(l, m, n, o, p, q)                                    \
	p += ((m & (n ^ o)) ^ o) + blk(q) + 0x5a827999 + rol(l, 5); \
	m = rol(m, 30);
#define R2(l, m, n, o, p, q)                            \
	p += (m ^ n ^ o) + blk(q) + 0x6ed9eba1 + rol(l, 5); \
	m = rol(m, 30);
#define R3(l, m, n, o, p, q)                                          \
	p += (((m | n) & o) | (m & n)) + blk(q) + 0x8f1bbcdc + rol(l, 5); \
	m = rol(m, 30);
#define R4(l, m, n, o, p, q)                            \
	p += (m ^ n ^ o) + blk(q) + 0xca62c1d6 + rol(l, 5); \
	m = rol(m, 30);
...
__device__ inline void sha1_expand(uint8_t *data, int *data_len)
{
	...
	*data_len = ((*data_len) + 8) / 64 * 64 + 56;
	...
	data[(*data_len)] = (m1 >> 56) & 0xff;
	data[++(*data_len)] = (m1 >> 48) & 0xff;
	data[++(*data_len)] = (m1 >> 40) & 0xff;
	data[++(*data_len)] = (m1 >> 32) & 0xff;
	data[++(*data_len)] = (m1 >> 24) & 0xff;
	data[++(*data_len)] = (m1 >> 16) & 0xff;
	data[++(*data_len)] = (m1 >> 8) & 0xff;
	data[++(*data_len)] = (m1) & 0xff;
...
__global__ void run_set(uint8_t *data_input, uint8_t *nonce, uint8_t *nonce_found, uint32_t *result_found, int data_len, int data_len_padded, int nonce_len, int epoch_count)
{
	...
	for (int i = 0; i < epoch_count; ++i)
	{
		...
		for (int j = range_lower; j < range_upper; ++j)
		{
			...
			if (result[0] < result_found[kernel_id * 5 + 0] || (result[0] == result_found[kernel_id * 5 + 0] &&
																(result[1] < result_found[kernel_id * 5 + 1] || (result[1] == result_found[kernel_id * 5 + 1] &&
																												 (result[2] < result_found[kernel_id * 5 + 2] || (result[2] == result_found[kernel_id * 5 + 2] &&
																																								  (result[3] < result_found[kernel_id * 5 + 3] || (result[3] == result_found[kernel_id * 5 + 3] &&
																																																				   result[4] < result_found[kernel_id * 5 + 4]))))))))
			{
